...
const BarChart3 = () => {
  ...
  const formatData = (data) => {
    if (!data) return []; // Manejar caso donde data es null o undefined
    
    return Object.keys(data)
      .slice(0, 10) // Tomar solo los últimos 10 días por ejemplo
      .reverse() // Invertir el orden para que los datos más recientes estén al final
      .map(date => ({
        date,
        value: parseFloat(data[date]['4. close']), // Tomar el precio de cierre ajustado
      }));
  };
